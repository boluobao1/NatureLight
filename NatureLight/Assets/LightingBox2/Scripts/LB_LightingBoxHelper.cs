...
public class LB_LightingBoxHelper : MonoBehaviour {
	...
	public void Update_RenderPath(bool enabled, Render_Path renderPath,Camera mainCamera,bool lightweight)
	{
		if (enabled) {
			if(lightweight)
			{
	...
	public void Update_GlobalFog(Camera mainCamera, bool enabled, CustomFog fogMode,float fogDistance,float fogHeight,float fogHeightDensity,Color fogColor,float fogDensity,bool lightweight)
	{
		if (enabled)
		{
			if (fogMode == CustomFog.Distance)
				UpdateFog (mainCamera, 1, fogDistance, fogHeight, fogHeightDensity, fogColor, fogDensity,lightweight);
			if (fogMode == CustomFog.Global)
				UpdateFog (mainCamera, 2, fogDistance, fogHeight, fogHeightDensity, fogColor, fogDensity,lightweight);
			//-----Height--------------------------------------------------------------------
			if (fogMode == CustomFog.Height)
				UpdateFog (mainCamera, 0, fogDistance, fogHeight, fogHeightDensity, fogColor, fogDensity,lightweight);
			//-----Global Fog Type--------------------------------------------------------------------
		...
		if (!enabled)
			UpdateFog(mainCamera,3,fogDistance,fogHeight,fogHeightDensity,fogColor,fogDensity,lightweight);
	}

	private void UpdateFog(Camera mainCamera,int fogType,float fogDistance,float fogHeight,float fogHeightDensity,Color fogColor,float fogDensity,bool lightweight) // 0 Height , 1 Distance , 2 Global , 3 Off
	{
		...
		{
			if(mainCamera.gameObject.GetComponent<GlobalFog> ())
				GameObject.DestroyImmediate(mainCamera.gameObject.GetComponent<GlobalFog> ());
