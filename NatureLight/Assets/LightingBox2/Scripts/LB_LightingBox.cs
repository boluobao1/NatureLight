...
public class LB_LightingBox : EditorWindow
{
	...
	public bool Lightweight_Target = false;
	...
	void OnGUI()
	{
		...
		if (winMode == WindowMode.Part1) {
			...
			if (profileState) {
				...
				var Lightweight_TargetRef = Lightweight_Target;
				EditorGUILayout.BeginHorizontal();

				webGL_Mobile = EditorGUILayout.Toggle ("WebGL 2.0 Target", webGL_Mobile);
				Lightweight_Target = EditorGUILayout.Toggle ("Lightweight Target", Lightweight_Target);
				EditorGUILayout.EndHorizontal();
				...
				if (Lightweight_TargetRef != Lightweight_Target) {
					UpdateSettings ();
					UpdatePostEffects ();
					if (LB_LightingProfile)
					{
						LB_LightingProfile.Lightweight_Target = Lightweight_Target;
						EditorUtility.SetDirty (LB_LightingProfile);
					}
				}
				EditorGUILayout.Space ();
			...
			var renderPathRef = renderPath;
				if(Lightweight_Target && renderPath == Render_Path.Deferred)
				{
					EditorGUILayout.LabelField ("Deferred rendering path is not supported o0n Lightweight pipeline", redStyle);			
			...
			if (renderPathRef != renderPath) {
					...
					helper.Update_RenderPath (Scene_Enabled,renderPath, mainCamera,Lightweight_Target);
			...
			{
				helper.Update_GlobalFog(mainCamera,Fog_Enabled,vFog,fDistance,fHeight,fheightDensity,fColor,fogIntensity,Lightweight_Target);
				if (LB_LightingProfile)
			...
			if (vFog == CustomFog.Distance || vFog == CustomFog.Height) {
				if( Lightweight_Target == true)
					EditorGUILayout.LabelField ("Distance and Height fog is not supported on Lightweight", redStyle);			
		...
		if (LB_LightingProfile) {
			...
			// Lightweight
			Lightweight_Target = LB_LightingProfile.Lightweight_Target;
